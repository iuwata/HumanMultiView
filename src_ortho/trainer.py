...
class HMRTrainer(object):
    def __init__(self, config, data_loader, mocap_loader):
        ...
        if self.use_3d_label:
            ...
            self.oripose_loader = data_loader['oripose']
        ...
        self.e_pose_weight = config.e_pose_weight
        self.e_shape_weight = config.e_shape_weight
    ...
    def build_model(self):
        ...
        loss_kps = []
        if self.use_3d_label:
            loss_3d_joints, loss_3d_pose, loss_3d_shape = [], [], []
        # For discriminator
        fake_rotations, fake_shapes = [], []
        # Start loop
        # 85D
        self.num_views = 4
        theta_prior = self.load_mean_param()
        theta_prev = [theta_prior for _ in range(self.num_views)]
        ...
        self.all_theta_prev = []
        self.all_gt_verts = []
        self.all_has_gt3d_smpl = []
        self.kp_copy = []

        # Main IEF loop
        for stage in np.arange(self.num_stage):
            print('Iteration %d' % stage)
            for i in np.arange(self.num_views):
                # ---- Compute outputs
                j = i
                ...
                state = tf.concat([self.img_feat[j], theta_prev[i]], 1)
                ...
                comp_kp = batch_orth_proj_idrot(gtJs, cams, name='gtproj2d_stage%d'%i)
                # visualize synthetic kp
                self.kp_copy.append(tf.gather(self.kp_loader[:,j,:,:]
                    # self.kp_loader[:,j,:,:2]*self.kp_loader[:,j,:,2:3]
                    # +comp_kp*(1-self.kp_loader[:,j,:,2:3])
                    , self.show_these))
                # --- Compute losses:
                loss_kps.append(self.e_loss_weight * self.keypoint_loss(
                    self.kp_loader[:,j,:,:], pred_kp))
                pred_Rs = tf.reshape(pred_Rs, [-1, 24, 9])
                if self.use_3d_label:
                    loss_pose, loss_shape, loss_joints = self.get_3d_loss(
                        pred_Rs, shapes, Js, j)
                    loss_3d_pose.append(loss_pose)
                    loss_3d_shape.append(loss_shape)
                ...
                self.all_gt_verts.append(tf.gather(gt_verts, self.show_these))
                self.all_has_gt3d_smpl.append(tf.gather(self.has_gt3d_smpl, self.show_these))
        ...
        with tf.name_scope("gather_e_loss"):
            ...
            if self.use_3d_label:
                self.e_loss_3d_pose = tf.reduce_mean(loss_3d_pose[-self.num_views:])
                self.e_loss_3d_shape = tf.reduce_mean(loss_3d_shape[-self.num_views:])
                self.e_loss_3d_joints = tf.reduce_mean(loss_3d_joints[-self.num_views:])

                self.e_loss += (self.e_loss_3d_pose + self.e_loss_3d_shape + self.e_loss_3d_joints)
        ...
        self.all_gt_verts = tf.stack(self.all_gt_verts, axis=1)
        self.all_has_gt3d_smpl = tf.stack(self.all_has_gt3d_smpl, axis=1)
        ...
        self.show_kps = tf.stack(self.kp_copy[-self.num_views:], axis=1)
    ...
    def setup_summaries(self, loss_kps):
        ...
        for i in np.arange(self.num_stage):
            ...
            always_report.append(
                tf.summary.scalar(name_here, tf.reduce_mean(loss_kps[i*4:i*4+4]) / self.e_loss_weight))
        if self.use_3d_label:
            always_report.append(
                tf.summary.scalar("loss/e_loss_3d_pose_noscale",
                                  self.e_loss_3d_pose / self.e_pose_weight))
            always_report.append(
                tf.summary.scalar("loss/e_loss_3d_shape_noscale",
                                  self.e_loss_3d_shape / self.e_shape_weight))
            always_report.append(
    ...
    def get_3d_loss(self, Rs, shape, Js, view_id):
        """
        ...
        # 24*9+10 = 226
        gt_params = self.poseshape_loader[:, view_id, :226]
        # noise = tf.random_uniform([self.batch_size, 10], minval=-0.2,maxval=0.2)
        loss_pose = self.e_pose_weight * compute_3d_loss(
            params_pred[:, :216], gt_params[:, :216], self.has_gt3d_smpl) * (216./226.)
        loss_shape = self.e_shape_weight * compute_3d_loss(
            params_pred[:, 216:], gt_params[:, 216:], self.has_gt3d_smpl) * (10./226.)
        # loss_pose = 0
        # 14*3 = 42
        gt_joints = self.poseshape_loader[:, view_id, 226:]
        pred_joints = Js[:, :14, :]
    ...
    def visualize_img(self, img, gt_kp, vert, pred_kp, cam, renderer, gt_vert, has_gt3d_smpl):
        """
    ...
    def draw_results(self, result):
        ...
        # B x T x 19 x 2
        jointss = result["joints"]
        # B x T x 3
        camss = result["cam"]
        gt_vertss = result["gt_verts"]
        has_gt3d_smplss = result["has_gt3d_smpl"]

        img_summaries = []

        for img_id, (img, gt_kp, verts, joints, cams, gt_verts, has_gt3d_smpls) in enumerate(
    ...
    def train(self):
        ...
        with self.sv.managed_session(config=self.sess_config) as sess:
            while not self.sv.should_stop():
                ...
                if self.use_3d_label:
                    fetch_dict.update({
                        "loss_3d_pose": self.e_loss_3d_pose,
                        "loss_3d_shape": self.e_loss_3d_shape,
                ...
                if step % self.log_img_step == 0:
                    fetch_dict.update({
                        ...
                        "gt_verts": self.all_gt_verts,
                        "has_gt3d_smpl": self.all_has_gt3d_smpl,
                    })
                    if not self.encoder_only:
                        fetch_dict.update({
                            "summary_occasional":
