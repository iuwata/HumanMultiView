...
def parse_example_proto(example_serialized, num_view, has_3d=True):
    ...
    feature_map = {
        'image/encoded':
        tf.FixedLenFeature([num_view], dtype=tf.string, default_value=['' for _ in range(num_view)]),
        'image/height':
        tf.FixedLenFeature([num_view, 1], dtype=tf.int64, default_value=[-1 for _ in range(num_view)]),
        'image/width':
        tf.FixedLenFeature([num_view, 1], dtype=tf.int64, default_value=[-1 for _ in range(num_view)]),
        'image/filename':
        tf.FixedLenFeature([num_view], dtype=tf.string, default_value=['' for _ in range(num_view)]),
        'image/center':
        tf.FixedLenFeature((num_view, 2, 1), dtype=tf.int64),
        'image/visibility':
        tf.FixedLenFeature((num_view, 1, 14), dtype=tf.int64),
        'image/x':
    ...
    if has_3d:
        feature_map.update({
            ...
            'meta/has_3d':
            tf.FixedLenFeature((num_3d,), dtype=tf.int64, default_value=[0 for _ in range(num_3d)]),
        })
    ...
    if has_3d:
        ...
        gt3d = tf.reshape(
            tf.cast(features['mosh/gt3d'], dtype=tf.float32), [num_view, 14, 3])
        has_smpl3d = tf.cast(features['meta/has_3d'], dtype=tf.bool)
        return image, image_size, label, center, fname, pose, shape, gt3d, has_smpl3d, has_3djoint
    else:
...
def get_all_files(dataset_dir, datasets, split='train'):
    ...
    diff_name = ['h36m', 'mpi_inf_3dhp', 'synthetic']
    ...
    if 'h36m' in datasets:
        data_dirs.append(
            join(dataset_dir, 'human36m_wjoints', split,
                 '*.tfrecord'))
    ...
    if 'synthetic' in datasets:
        data_dirs.append(
            join(dataset_dir, 'synthetic', split, '*.tfrecord'))
...
def jitter_scale(image, image_size, keypoints, center, scale_range, output_size, scale_factor=None):
    with tf.name_scope(None, 'jitter_scale', [image, image_size, keypoints]):
        ...
        # new_size = tf.maximum(new_size, tf.constant([output_size, output_size]))
